...
def ApplyMaxFilter(PaddedImage):
    ...
    for r in range(1,PaddedImage.shape[0]-1):
        for c in range(1,PaddedImage.shape[1]-1):
            ...
            Max = CalculateMax(Kernel)
            FilteredImage[r, c] = Max
...
FilteredImage = ApplyMaxFilter(PaddedImage)

FilteredImage = Int32ToUint8(FilteredImage)

SaveImage('Output Images\Max Filter.png',FilteredImage)